...
def new_project(name, width, height)
	...
	FileUtils.cp('./lib/htmltemplate', name + '/lib')
	FileUtils.cp('./lib/xc.js' , name + '/lib')
	FileUtils.cp('./lib/jquery-1.4.2.min.js' , name + '/lib')
	FileUtils.cp('./lib/xc_canvas.js' , name + '/lib')
	FileUtils.cp('./lib/xc_ios.js' , name + '/lib')
	
	
	FileUtils.cp('./lib/resources/man.png', name + '/resources')
	
	FileUtils.cp('./lib/main.js' , name)
	
	FileUtils.cp('./xc.rb', name)
